...
function editemp($id){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM emp_registration  where id='".$id."'";
	$stmt= $mysqli->query($query);
	return $stmt;
...
function register($cshort,$cfull,$fname,$gender,$gname,$category,
                  $mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
		           ...
		           //	echo $session;exit;
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`,  `gender`, `gname`, `category`, `mobno`, `emailid`, `country`, `state`, `dist`, 
...
//register employee or insert emp data into database//


function emp_register($cshort,$cfull,$fname,$gender,$gname,$category,
                 $mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$sub1,$sub2,$marks1,$marks2,$fmarks1,$fmarks2,$session){
		           ...
		           //	echo $session;exit;
   $query = "INSERT INTO `emp_registration` (`course`, `subject`, `fname`,`gender`, `gname`,`category`,`mobno`, `emailid`, `country`, `state`, `dist`,`padd`,`cadd`, `board`, `board1`,`roll`,`roll1`,`pyear`,`yop1`,`sub`,`sub1`,`marks`,`marks1`,
					 `fmarks`,`fmarks1`,`session`,regno) 
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			        $reg=rand();
			        $stmt= $mysqli->prepare($query);
			...
			$stmt->bind_param('sssssssssssssssssssssssssss',
		         	$cshort,$cfull,$fname,$gender,$gname,$category,$mobno,
					$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,$pyear1,$pyear2,
					$sub1,$sub2,$marks1,$marks2,$fmarks1,$fmarks2,$session,$reg);
			$stmt->execute();
		   	echo "<script>alert('Successfully Registered , your registration number is $reg')</script>";
...
//ending function of add emp data////
